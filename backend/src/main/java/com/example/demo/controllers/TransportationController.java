...
public class TransportationController {
  ...
  public TransportationController(
      TransportationRepository transportationRepository, LocationRepository locationRepository) {
  ...
  public ResponseEntity<TransportationDTO> createTransportation(
      @Valid @RequestBody TransportationCreateDTO transportationCreateDTO) {
    Location originLocation =
        locationRepository
            .findById(transportationCreateDTO.getOriginLocationId())
            .orElseThrow(
                () ->
                    new IllegalArgumentException(
                        "Origin location not found for ID: "
                            + transportationCreateDTO.getOriginLocationId()));
    Location destinationLocation =
        locationRepository
            .findById(transportationCreateDTO.getDestinationLocationId())
            .orElseThrow(
                () ->
                    new IllegalArgumentException(
                        "Destination location not found for ID: "
                            + transportationCreateDTO.getDestinationLocationId()));

    Transportation transportation =
        TransportationMapper.toEntity(transportationCreateDTO, locationRepository);
  ...
  public ResponseEntity<TransportationDTO> updateTransportation(
      @PathVariable Long id, @RequestBody TransportationCreateDTO updatedTransportation) {
    return transportationRepository
        .findById(id)
        .map(
            existingTransportation -> {
              if (updatedTransportation.getOriginLocationId() != null) {
                Location originLocation =
                    locationRepository
                        .findById(updatedTransportation.getOriginLocationId())
                        .orElseThrow(
                            () ->
                                new IllegalArgumentException(
                                    "Origin location not found for ID: "
                                        + updatedTransportation.getOriginLocationId()));
                existingTransportation.setOriginLocation(originLocation);
              }
              if (updatedTransportation.getDestinationLocationId() != null) {
                Location destinationLocation =
                    locationRepository
                        .findById(updatedTransportation.getDestinationLocationId())
                        .orElseThrow(
                            () ->
                                new IllegalArgumentException(
                                    "Destination location not found for ID: "
                                        + updatedTransportation.getDestinationLocationId()));
                existingTransportation.setDestinationLocation(destinationLocation);
              }
              if (updatedTransportation.getTransportationType() != null) {
                existingTransportation.setTransportationType(
                    updatedTransportation.getTransportationType());
              }
  ...
  public ResponseEntity<List<TransportationDTO>> createTransportations(
    ...
    List<Transportation> transportations =
        transportationCreateDTOs.stream()
            .map(
                dto -> {
                  Location originLocation =
                      locationRepository
    ...
    List<TransportationDTO> response =
        savedTransportations.stream().map(TransportationMapper::toDto).collect(Collectors.toList());
