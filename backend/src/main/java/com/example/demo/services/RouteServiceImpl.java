...
public class RouteServiceImpl implements RouteService {
  ...
  public RouteServiceImpl(
      LocationRepository locationRepository, TransportationRepository transportationRepository) {
  ...
  @Cacheable(
      value = "routes",
      key = "#originLocationCode + '_' + #destinationLocationCode + '_' + #travelDate.toString()")
  public List<RouteDTO> findMatchingRoutes(
      String originLocationCode, String destinationLocationCode, LocalDate travelDate) {
    Location originLocation =
        locationRepository
            .findByLocationCode(originLocationCode)
            .orElseThrow(
                () ->
                    new IllegalArgumentException(
                        "Location with code " + originLocationCode + " not found"));
    Location destinationLocation =
        locationRepository
            .findByLocationCode(destinationLocationCode)
            .orElseThrow(
                () ->
                    new IllegalArgumentException(
                        "Location with code " + destinationLocationCode + " " + "not found"));
    ...
    List<Long> originIds =
        beforeAirports.stream().map(Location::getId).collect(Collectors.toList());
    List<Long> destinationIds =
        afterAirports.stream().map(Location::getId).collect(Collectors.toList());
      ...
      for (Transportation beforeTransfer : beforeTransfers) {
        for (Transportation afterTransfer : afterTransfers) {
          if (beforeTransfer
                  ...
                  .getId()
                  .equals(usableFlight.getOriginLocation().getId())
              && afterTransfer
                  .getOriginLocation()
                  .getId()
                  .equals(usableFlight.getDestinationLocation().getId())) {
      ...
      for (Transportation usableFlight : usableFlights) {
        for (Transportation afterTransfer : afterTransfers) {
          if (usableFlight
                  .getDestinationLocation()
                  .getId()
                  .equals(afterTransfer.getOriginLocation().getId())
      ...
      for (Transportation usableFlight : usableFlights) {
        for (Transportation beforeTransfer : beforeTransfers) {
          if (usableFlight
                  .getOriginLocation()
                  .getId()
                  .equals(beforeTransfer.getDestinationLocation().getId())
              && usableFlight
                  .getDestinationLocation()
                  .getId()
                  .equals(destinationLocation.getId())) {
            StopDTO stop1 = new StopDTO();
            stop1.setLocation(LocationMapper.toDto(beforeTransfer.getOriginLocation()));
            stop1.setStopNumber(1);
            stop1.setTransportationTypeToNext(beforeTransfer.getTransportationType());
